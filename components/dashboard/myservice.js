  ...
  useEffect(() => {
    ...
    (async () => {
      if (token) {
        ...
        try {
          if (data.data) {
            let { NotificacionsClient } = data.data;
            if (NotificacionsClient) {
              Setloading(false);
              SetData(NotificacionsClient);
              console.log(NotificacionsClient);
            } else {
              console.log("err");
            }
          } else {
      ...
      renderItem={(item) => (
        <List.Item>
          <List.Item.Meta
            ...
            title={<Link href={{pathname:"/service", query: { idservice: item._id }}} as={`/service/${item._id}/`} ><a >{item.titulo} - S/. {item.precio} </a></Link>}
            description={
              <ContentR>
                <Time>
                ...
                <Cont>{item.descripcion.detalles}</Cont>
              </ContentR>
            }
          />

          {item.subasta.length !== 0 && (
            <ComentaryList
              className=""
              itemLayout="vertical"
